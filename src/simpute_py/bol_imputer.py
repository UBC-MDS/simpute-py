def bol_imputer(df,col,method='mode'): #How is the user converting their data into a df? 
    ...
    method: string
        A name of the imputing strategy. They include ['mode','rand'], the default method is method.
    Returns
    ----------
    y: pandas dataframe
    ...
    if bool_col.isnull().sum() == 0:
        return print("Your column does not have any missing values to impute.")

    #Check if the column is boolean
    if (bool_col.dropna().isin([True, False, 1, 0, 'True', 'False'])).all() == False:
        return print("You column is not Boolean, please select another column.")
